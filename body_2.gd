...
@onready var trail := $"../Trail2"
func _ready():
	
	#color
	self_modulate = Global.body2color
	
	#pos
	realpos = Vector2(Global.body2posx, Global.body2posy)
	print(Global.scale)
	allbodies = [$"../Body1", $"../Body2", $"../Body3"]
	self.scale = self.scale / Vector2(Global.zoom, Global.zoom)
	update_screen_pos()
	velocity = Global.body2velocity
	trail.width = 1.5
...
func _process(delta: float) -> void:
	...
	for i in allbodies:
		...
		if i == $"../Body1":
			#force equation
			force = ((Global.body1masskg * Global.body2masskg) / (distance * distance)) * Global.G
		else:
			force = ((Global.body3masskg * Global.body2masskg) / (distance * distance)) * Global.G
		var acceleration_vector = direction * force / Global.body2masskg
